 ...
 *
 */
public class GuestOSMXBeanTest_Remote extends TestCase{
	...
	private static  MBeanServerConnection mbeanConnection = null;
	private static  JMXServiceURL urlForRemoteMachine = null;
	...
	static {
		try {
			...
			// Connect to remote host and create a proxy to be used to get an GuestOSMXBean instance.
		...
		try {
            ...
            if (bean.getVendor().contains("PowerKVM")) {
				...
				if ((-1!= maxMemLimit) && (-1 != memUsed)) {
					if ((memUsed > 0) && (memUsed <= maxMemLimit)) {
						// ok
					} else {
					}
				} else {
					if (-1 == memUsed) {
						...
						fail("Invalid Current Used Memory by the Guest reported, Memory Used cannot be less than -1!!:memUsed:" + memUsed);
					} else {
						//ok
					}
					...
					} else if (maxMemLimit < -1){ 
						fail("Maximum Memory that can be used by the Guest:maxMemLimit" + maxMemLimit);
					} else {
						//ok
					}
				}	
